...
const GoogleInUpdateExistingUserData = () => {
	...
	const enableGoogleInUpdateExistingUserData = useSelector((state) => state.enableGoogleInUpdateExistingUserData);
	const enableGoogleInUpdateExistingUserDataStatus = false === enableGoogleInUpdateExistingUserData ? false : true;

	const updateLogsStatus = () => {

		let assetStatus;
		if (enableGoogleInUpdateExistingUserData === false) {
			assetStatus = true;
		} else {
			assetStatus = false;
		}
	...
	return (
		<section className='block border-b border-solid border-slate-200 px-8 py-8 justify-between'>
			<div className='mr-16 w-full flex items-center'>
				...
				<Switch
					checked={enableGoogleInUpdateExistingUserDataStatus}
					onChange={updateLogsStatus}
					className={classNames(
						enableGoogleInUpdateExistingUserDataStatus ? 'bg-lmn' : 'bg-slate-200',
				...
				>
					...
					<span
						aria-hidden="true"
						className={classNames(
							enableGoogleInUpdateExistingUserDataStatus ? 'bg-lmn' : 'bg-gray-200',
					...
					<span
						aria-hidden="true"
						className={classNames(
							enableGoogleInUpdateExistingUserDataStatus ? 'translate-x-5' : 'translate-x-0',
